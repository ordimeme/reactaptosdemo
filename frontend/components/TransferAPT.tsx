...
export function TransferAPT() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      Recipient <Input disabled={!account} placeholder="0x1" onChange={(e) => setRecipient(e.target.value)} className="bg-[var(--background)] text-[var(--textColor)] border-[var(--softBg)] placeholder:text-[var(--softTextColor)]"/>
      Amount{" "}
      <Input disabled={!account} placeholder="100" onChange={(e) => setTransferAmount(parseFloat(e.target.value))} className="bg-[var(--background)] text-[var(--textColor)] border-[var(--softBg)] placeholder:text-[var(--softTextColor)]" />
      <Button
